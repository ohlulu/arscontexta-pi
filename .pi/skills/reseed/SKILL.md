...
3. **Query the research graph** -- use `qmd dsearch` to search for claims relevant to the friction. Fall back to `qmd vsearch`. If MCP is unavailable, use qmd CLI (`qmd query`, then `qmd vsearch`). Fall back to reading bundled reference files directly only if both MCP and qmd CLI are unavailable.
