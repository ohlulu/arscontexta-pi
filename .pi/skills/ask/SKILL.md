...
## Vault Path Resolution (Step 0)

> **Cross-project support:** This skill operates on a vault that may not be in the current working directory.
> Read vault path from `~/.config/arscontexta.yaml` (key: `default_vault`), or detect `.arscontexta` marker in cwd.
> All vault content paths (notes/, ops/, self/, inbox/) are relative to this vault root.
> The session-orient extension injects the active vault path at session start — check conversation context first.
...
3. **Read relevant claims and docs** — load 3-7 most relevant sources fully (use `qmd mget` when reading multiple IDs)
...
**Search strategy:** Use `qmd dsearch` (highest quality, LLM-reranked) for conceptual questions. Use `qmd vsearch` for semantic exploration. Use `qmd search` for known terminology. All searches use the `methodology` collection.
...
**Search strategy:** `qmd search` with keywords from the question using the `methodology` collection. To narrow to guidance docs, add `kind:guidance` to your grep filter on results.
...
```
qmd dsearch  query="[user's question rephrased as a search]"  collection="methodology"  limit=10
```
Use `qmd dsearch` (hybrid + LLM reranking) for conceptual questions because the best connections often use different vocabulary than the question. Results will include all kinds; prioritize `kind: research` results.

**For HOW questions (Guidance Docs):**
```
qmd search  query="[key terms from question]"  collection="methodology"  limit=5
```
...
```
qmd vsearch  query="[domain + what the user wants to see]"  collection="methodology"  limit=5
```
...
**Search:**
1. `qmd dsearch  query="atomic notes vs compound documents granularity"  collection="methodology"  limit=8`
...
**Search:**
1. `qmd search  query="large source processing chunking"  collection="methodology"  limit=5`
2. `qmd dsearch  query="context degradation large documents extraction"  collection="methodology"  limit=5`
...
**Search:**
1. `qmd vsearch  query="cooking recipes culinary knowledge system"  collection="methodology"  limit=5`
