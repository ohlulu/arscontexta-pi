...
```
qmd dsearch  query="[user's domain] knowledge management patterns"
qmd vsearch  query="[user's specific concern or goal]"
```
...
For each search result that looks relevant, read the claim via `qmd get` (or `qmd mget` for batch reads) to understand the full argument. You need enough depth to cite with confidence.
